...
void RunKernels::dataInitialization()
{
	...
	if (inputFileInitH.fail() ){
		cout << "\nError can't open Initial depth file.\n"<< endl;
		assert(0);
	}

	if (inputFileInitU.fail() ){
		cout << "\nError can't open Initial velocity(U) file.\n"<< endl;
		assert(0);
	}

	if (inputFileInitV.fail() ){
		cout << "\nError can't open Initial velocity(V) file.\n"<< endl;
		assert(0);
	}

	if (inputFileloc.fail() ){
		cout << "\nError can't open Inflow location file.\n"<< endl;
		assert(0);
	}
...
clock_t RunKernels::run()
{
	...
	cudaMallocHost((void **)&inflow_h, sizeof(double) * inflowSize * 3);
	cudaMallocHost((void **)&loc_h, sizeof(double) * locflowlen * 3);
	cudaMallocHost((void **)&dire_h, sizeof(int) * locflowlen * 3);
...
void RunKernels::kernelStep()
{
	...
	cudaMemset(dev_dt, 0, sizeof(double));
	cudaMemset(dev_dtval, 0, sizeof(double));
	
	cudaMemcpy(dev_topo,   topo_h, sizeof(double) * arraySize, cudaMemcpyHostToDevice);
	cudaMemcpy(dev_inflow,   inflow_h, sizeof(double) * (inflowSize * 3), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_loc,      loc_h,    sizeof(double) * (locflowlen * 3), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_dire,     dire_h,   sizeof(int)    * (locflowlen * 3), cudaMemcpyHostToDevice);
	...
	for (int nt = 1; (!nstop) && (nt<1000000); nt++){
		...
		for (int io=0; io<2; io++){
			...
			if(io==0){
				...
				int threads = 256;//512;
				int blocks = 256;//min((arraySize + threads - 1) / threads, 512);
...
void RunKernels::memoryMalloc()
{
	...
	cudaMalloc((void **)&dev_inflow, sizeof(double) * (inflowSize * 3));
	cudaMalloc((void **)&dev_loc, sizeof(double) * (locflowlen * 3));

	cudaMalloc((void **)&dev_dire, sizeof(int) * (locflowlen * 3));
}
