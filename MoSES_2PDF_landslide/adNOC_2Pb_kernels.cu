    ...
    int nxd, int nyd)
{
    ...
    if (row < (nyd) && col < (nxd) && row >=0 && col >=0 ) {
        ...
        u[0 * nxd * nyd + row * nxd + col] = max(0.0, u[0 * nxd * nyd + row * nxd + col]);

        u[3 * nxd * nyd + row * nxd + col] = max(0.0, u[3 * nxd * nyd + row * nxd + col]);
    ...
    int nxd, int nyd)
{
    ...
    // result[row * nxd + col] = invJ22[row * nxd + col];
